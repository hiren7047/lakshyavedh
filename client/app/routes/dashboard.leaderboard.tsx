import { storage, type Game } from "../lib/storage";
...
export default function Leaderboard() {
  ...
  const visibleGames = isUserAdmin ? games : games.filter(g => {
    ...
    // Show completed games or games where user's room is completed
  ...
  return (
    <main className="max-w-7xl mx-auto p-6 space-y-6">
      <div className="text-center">
        <SectionTitle>🏆 Game Leaderboard</SectionTitle>
      ...
      {visibleGames.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {visibleGames.map((game) => {
            ...
            return (
              <Card key={game.id} className={`bg-white/90 backdrop-blur-sm hover:shadow-2xl transform hover:scale-105 transition-all duration-300 ${
